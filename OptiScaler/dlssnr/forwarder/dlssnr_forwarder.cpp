...
void recordModelError(Snippet* snippet, const char* fallback)
{
    auto message = snippet && snippet->module
                       ? (const char* (*) ()) GetProcAddress(snippet->module, "NVFP4_GetLastError")
                       : nullptr;
...
extern "C"
{
                                                        ...
                                                        unsigned int* outOsVersion)
    {
        ...
        auto req = (PFN_NrD3D11Requirements) GetProcAddress(g_d3d11.module, "NVSDK_NGX_D3D11_GetFeatureRequirements");
    ...
    __declspec(dllexport) int dlssnr_d3d11_init(const wchar_t* snippetPath, const wchar_t* dataPath, void* device,
                                                int sdkVersion, int* attemptOut, int* resultsOut)
    {
        if (device == nullptr)
        {
            return -1;
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (last == 1)
            {
                ...
                g_d3d11.create =
                    (PFN_NrD3D11Create) GetProcAddress(attempts[i].module, "NVSDK_NGX_D3D11_CreateFeature");
                g_d3d11.evaluate =
                    (PFN_NrD3D11Evaluate) GetProcAddress(attempts[i].module, "NVSDK_NGX_D3D11_EvaluateFeature");
                g_d3d11.release = (PFN_NrRelease) GetProcAddress(attempts[i].module, "NVSDK_NGX_D3D11_ReleaseFeature");
    ...
    __declspec(dllexport) void* dlssnr_d3d11_create(void* deviceContext, void* capabilityParams, unsigned int width,
    ...
    __declspec(dllexport) int dlssnr_vk_init(const wchar_t* snippetPath, const wchar_t* dataPath, void* instance,
                                             void* physicalDevice, void* device, int sdkVersion)
    {
    ...
    __declspec(dllexport) int dlssnr_vk_evaluate_v2(
                                                   ...
                                                   int uiCorrection)
    {
        ...
        if (dlssnr_call_last_create == 1 && handle)
            g_featureOwners[handle] = snippet;
        else
            recordModelError(snippet, "model feature creation failed");
    ...
    __declspec(dllexport) int dlssnr_call_evaluate_v2(
        ID3D12GraphicsCommandList* cmd, void* feature, void* capabilityParams, ID3D12Resource* color,
        ID3D12Resource* depth, ID3D12Resource* motion, ID3D12Resource* output, unsigned int width, unsigned int height,
        unsigned int guideWidth, unsigned int guideHeight, unsigned int motionWidth, unsigned int motionHeight,
        unsigned int depthBaseX, unsigned int depthBaseY, unsigned int motionBaseX, unsigned int motionBaseY,
        int depthInverted, int reset, float intensity, int style, float localStructure, float localTone,
        float skinStructure, int useAutoMask, float mvScaleX, float mvScaleY)
    {
        ...
        if (result != 1)
            recordModelError(owner->second, "model evaluation failed");
        return result;
    }
