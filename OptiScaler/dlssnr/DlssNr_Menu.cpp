...
void RenderMenu(Config* config, float menuResScale)
{
    ...
    if (auto ch = ScopedCollapsingHeader("DLSS Neural Rendering"); ch.IsHeaderOpen())
    {
        ...
        HelpMarker("Enhance lighting and material appearance with the NR model. Placement selects before or after "
        ...
        HelpMarker(
            ...
            "with frame generation on or off in native DirectX 12 games, including SDR, HDR10 and scRGB.\nIt can also "
            "change the HUD and menus. Enable Run the model before Super Resolution to generate the changes earlier.");
        ...
        const bool deferredActive =
            !finishedPicture && config->DlssNrDeferredDlss.value_or_default() && !rayReconstruction;
        if (deferredActive)
            ImGui::BeginDisabled();
        ...
        HelpMarker(finishedPicture ? "Run the model at the smaller input size, upscale its changes with DLSS, then "
                                     "apply them to the finished picture.\nExperimental: the colour transfer is "
                                     "approximate and may look different. Requires DLSS SR; does not support RR."
        ...
        if (!finishedPicture)
        {
            ...
            ImGui::EndDisabled();
            HelpMarker(
                "Only with Apply before Super Resolution on and the game's Ray Reconstruction active.\nRuns the model "
                "before SR but leaves the colour input untouched, then adds its edit back onto the RR+SR output so it "
                "survives RR's denoise.\nThe edit is carried as a motion-vector-reprojected temporal accumulator: the "
                "per-frame ray-trace noise averages out, the enhancement stays. Inert otherwise.");
            ...
            ImGui::EndDisabled();
            HelpMarker("How fast the carried edit builds up. Lower = stabler but slower to appear; 1.0 = no "
                       "accumulation (each frame's raw residual, which flickers). Default 0.08.");
        }
        ...
        HelpMarker(
            "NVIDIA: original FP8 model (default), with some sensitive operations kept at higher "
        ...
        if (!finishedPicture)
        {
            ...
            HelpMarker(
                "Compute NR at input resolution, upscale its changes with DLSS, then apply them after "
            ...
            if (deferredDlss && rayReconstruction)
                ImGui::TextWrapped(
                    "Generate before / apply after is unavailable with RR. Apply before Super Resolution "
            ...
            HelpMarker("Run NR every other rendered frame and use NVIDIA FG to interpolate its changes.\nRequires the "
                       "option above. Adds one rendered frame of latency and may misalign effects or UI.\nIf motion "
                       "vectors are unavailable, each NR result is reused for two frames.");
            ...
            HelpMarker("Use estimated camera data when the game does not provide it. May cause artifacts during camera "
                       "movement.");
            ImGui::EndDisabled();
        }
        else if (beforeSr)
        ...
        HelpMarker("Allow up to 30 passes instead of 3. More passes use more GPU time and VRAM; high values may crash "
                   "the game.");
        const unsigned int passLimit = unlockPasses ? MaxPassCount : DefaultMaxPassCount;

        {
            int passes = (int) std::clamp(config->DlssNrPasses.value_or_default(), 1u, passLimit);
        ...
        HelpMarker("NR resolution relative to the image it processes. 50% halves width and height; 100% uses the full "
                   "size.\nLower values reduce cost and fine detail. Above 100% increases cost. Game output resolution "
                   "is unchanged.");
        ...
        if (scalePercent > 100)
        {
            ...
            HelpMarker("Filter used to reduce NR output when Model resolution exceeds 100%.\nSharper filters may "
                       "introduce ringing around edges.");
        }
        ...
        // model's own picture and the two modes are identical, so the control says so by going grey.
        {
            ...
            HelpMarker("Below 100% model resolution: Classic enlarges the model output; Matched residual enlarges only "
        ...
        HelpMarker("Choose how HDR brightness is mapped for NR.\nSoft knee compresses highlights. Neutwo uses a "
                   "reversible curve. Hybrid preserves midtones and compresses highlights.\nComposed uses the strength "
                   "and highlight controls. Replace bypasses them and may flicker.");
        ...
        if (ImGui::TreeNodeEx("Pass 1", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            HelpMarker(
                "Select the appearance profile: Standard, Natural or Cinematic. Intensity controls its strength.");
            ...
            HelpMarker("Use the model's learned skin selection to apply Skin structure without an authored "
        ...
        for (unsigned int pass = 3; pass < visiblePasses; ++pass)
        {
            ...
            DeferredSlider("Intensity", &settings.intensity, 0.0f, 2.0f, config->DlssNrIntensity.value_or_default(),
                           "%.2f", true);
            DeferredSlider("Local structure", &settings.structure, 0.0f, 2.0f,
                           config->DlssNrLocalStructure.value_or_default(), "%.2f", true);
            DeferredSlider("Local tone", &settings.tone, 0.0f, 2.0f, 0.0f, "%.2f", true);
            DeferredSlider("Skin structure", &settings.skin, -1.0f, 2.0f,
                           config->DlssNrSkinStructure.value_or_default(), "%.2f", true);
        ...
        ImGui::TextWrapped(
            "Pass settings apply to SR and RR on DX12 and native Vulkan. The driver-proxy backend supports one pass.");
        ...
        if (ImGui::TreeNode("Skin and environment (final edit)"))
        {
            ...
            HelpMarker("Allow NR colour changes in the selected skin region. Turn off to preserve its colour; detail "
                       "can still change.");
...

        {
            ...
            // choice is made on what is available rather than on what sounds best.
            {
                ...
                if (source == 1)
                {
                    if (!vk && ex.seenFrames == 0)
                        ...
                        ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f), "Using game exposure.");
                ...
                else if (source == 2)
                {
                    ...
                    if (anchorNow <= 0.0f)
                    {
                        ...
                        if (watching == 0)
                            ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f), "No exposure candidates found.");
                        else
                            ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f),
                ...
                {
                    ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f), "Game exposure is available.");
                }
            }
            ...
            if (wpSource == 2)
            {
                ...
                if (showPaperWhite)
                {
                    ...
                    HelpMarker("Adjust the selected calibration point, or set the value for the next point.\nUse "
                               "Anchor here to save the current lighting condition.");
                }
                ...
                if (!anchors.empty())
                {
                    ...
                    HelpMarker("Multiply the calibrated white point. Anchor here saves the adjusted value and resets "
                               "this multiplier to 1.");
                }
            }
            else if (wpSource == 1)
            {
            ...
            else
            {
                ...
                HelpMarker("Brightness reference used to prepare HDR colour for NR. Higher values darken the model "
                           "input; lower values brighten it.\nAdjust if NR loses detail or produces colour shifts.");
            }
            ...
            if (ImGui::SliderFloat("Highlight guard", &maxRatio, 1.0f, unlockPasses ? (float) MaxPassCount : 8.0f,
                                   "%.1fx"))
            ...
            HelpMarker("Limit how much NR can brighten or darken a pixel. Lower values restrict highlight changes; "
            ...
            // reads, which left "Anchor here" looking like a control for something else entirely.
            {
                ...
                if (DlssNr::ExposureScan::Scanning())
                {
                    ...
                    HelpMarker("Save the current exposure and white point as a calibration point.\nAdjust Paper white "
                               "for the first point, then Trim for additional lighting conditions. Up to 8 points.");
                    ...
                    if (anchors.size() == 1)
                    {
                        ...
                        HelpMarker("Reverse how scanned exposure changes the white point. Only needed with one "
                                   "calibration point.");
                    }
        ...
        HelpMarker("Freeze NR's input to compare its settings. The game's HUD and later effects may keep "
        ...
        HelpMarker(
            "Compare the original and NR result. Side by side fits both images; Wipe divides one full-size image.");
        ...
        HelpMarker(
            "Show the model input, raw output, or a 20x amplified difference. Grey in Difference means no change.");
